...
# ---------- ADMIN PANEL ŞİFRESİ ----------
ADMIN_PASSWORD = "vexor2026"  # Değiştir!
...
def generate_codes(phone, count=50):
    """50 tane kod üret ve veritabanına kaydet"""
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    
    codes = []
    for _ in range(count):
        code = ''.join([str(random.randint(0, 9)) for _ in range(6)])  # 6 haneli
        codes.append(code)
...
def get_codes_by_phone(phone):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute('SELECT code, used, created_at FROM codes WHERE phone = ? ORDER BY created_at DESC', (phone,))
    result = c.fetchall()
    conn.close()
    return result
        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    ...
                    {% endfor %}
                    {% if not logs %}
                    <tr><td colspan="5" style="text-align:center; color:#4a6a8a;">Henüz log kaydı yok</td></tr>
                    {% endif %}
                </tbody>
            </table>
        </div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ code[1] }}</td>
                        <td>{{ code[2] }}</td>
                        <td>{% if code[4] %}❌ Kullanıldı{% else %}✅ Aktif{% endif %}</td>
                        <td>{{ code[5] }}</td>
                        <td>{{ code[3] }}</td>
                    </tr>
                    {% endfor %}
                    {% if not all_codes %}
                    <tr><td colspan="6" style="text-align:center; color:#4a6a8a;">Henüz kod oluşturulmamış</td></tr>
                    {% endif %}
                </tbody>
            </table>
        </div>
        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    ...
                    {% endfor %}
                    {% if not bans %}
                    <tr><td colspan="5" style="text-align:center; color:#4a6a8a;">Banlanmış IP yok</td></tr>
                    {% endif %}
                </tbody>
            </table>
        </div>
    ...
    if not session.get('admin_logged_in'):
        return render_template_string("""
            ...
            <head>
                ...
                <style>
                    body { background:#0a0a0f; font-family:'Segoe UI',sans-serif; display:flex; justify-content:center; align-items:center; min-height:100vh; margin:0; color:#d0d8f0; }
                    .login-box { max-width:400px; width:100%; background:#0d0d1a; border-radius:30px; padding:35px; border:1px solid #1e2d4a; }
                    h1 { color:#00ffb3; text-align:center; }
                    input { width:100%; padding:14px; background:#0b1222; border:2px solid #2a3a5c; border-radius:12px; color:#d0d8f0; margin:10px 0; }
                    .btn { width:100%; padding:14px; background:#00ffb3; color:#0a0a0f; border:none; border-radius:30px; font-weight:700; cursor:pointer; }
                    .btn:hover { background:#00cc99; }
                    .alert { background:#2a1a1a; border:1px solid #ff6b6b; border-radius:12px; padding:15px; margin:10px 0; color:#ff6b6b; text-align:center; }
                </style>
            </head>
            <body>
                <div class="login-box">
